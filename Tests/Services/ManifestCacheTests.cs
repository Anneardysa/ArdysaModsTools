...
{
    /// <summary>
    /// Covers the persisted-manifest cache (last-known-good heroes.json) and SHA-256 helpers that back
    /// the stale-database fix and the Settings → Hero Database check/update flow.
    /// </summary>
    [TestFixture]
    public class ManifestCacheTests
    {
        ...
        public void NormalizeJson_StripsBomAndCarriageReturns()
        {
            ...
            // Ordinal: StartsWith's default culture comparison treats U+FEFF as ignorable (always true).
        ...
        {
            string name = $"missing_{Guid.NewGuid():N}.json"; // never written
        ...
        public void CountSets_ExcludesDefaultSet_AndSumsAcrossHeroes()
        {
            ...
            // 2 (hero a) + 1 (hero b) = 3 non-default sets.
