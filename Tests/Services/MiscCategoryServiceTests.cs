...
{
    // Drives MiscCategoryService end-to-end by injecting a crafted config through the on-disk
    // cache (RemoteMiscConfigService reads it via GetConfigSync). Any real cache is preserved.
    [TestFixture]
    public class MiscCategoryServiceTests
    {
        ...
        public void GetAllOptions_StyleThumbnailId_FlowsThroughToOverride()
        {
            ...
            Assert.Multiple(() =>
            {
                // The style override is collected, keyed by style name.
                Assert.That(option.ChoiceThumbnailIds.ContainsKey("Golden Variant"), Is.True);
                // And it resolves to the shared stem instead of the sanitized style name.
