...
{
    /// <summary>
    /// Tests for GameItemsGameExtractorService guard paths (no HLExtract process launch).
    /// </summary>
    [TestFixture]
    public class GameItemsGameExtractorServiceTests
    {
        ...
        {
            // A real folder with no game/dota/pak01_dir.vpk underneath it.
            string tempTarget = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString());
            ...
            try
            {
                ...
                Assert.That(result, Is.False);
                // Should fail at the missing-game-VPK guard, never overwriting anything.
