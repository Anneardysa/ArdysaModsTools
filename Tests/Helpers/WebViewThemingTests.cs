...
namespace ArdysaModsTools.Tests.Helpers
{
    ...
    public class WebViewThemingTests
    {
        ...
        [Test]
        public void EveryHtmlAsset_IsSelfContained()
        {
            var htmlDir = LocateHtmlDir();
            ...
            foreach (var file in Directory.GetFiles(htmlDir, "*.html"))
            {
                ...
                if (name.Equals("dota2_performance.html", StringComparison.OrdinalIgnoreCase)) continue;

                var text = File.ReadAllText(file);

                Assert.That(Regex.IsMatch(text, @"<script[^>]*\ssrc\s*=", RegexOptions.IgnoreCase), Is.False,
                    $"{name} references an external script — unreachable under NavigateToString");
                Assert.That(Regex.IsMatch(text, @"<link[^>]*rel\s*=\s*[""']stylesheet", RegexOptions.IgnoreCase), Is.False,
                    $"{name} references an external stylesheet — unreachable under NavigateToString");
            }
        }

        [Test]
        public void ThemeCss_HasNoOrphanedComponentRules()
        {
            var htmlDir = LocateHtmlDir();
            var themeCss = StripCssComments(File.ReadAllText(Path.Combine(htmlDir, "theme.css")));

            var handWritten = Directory.GetFiles(htmlDir, "*.html")
                .Select(File.ReadAllText)
                .Where(text => !text.Contains("GENERATED by Frontend/build.mjs"))
                .ToList();

            var orphans = new List<string>();

            foreach (var match in Regex.Matches(themeCss, @"([^{}]+)\{([^}]*)\}").Cast<Match>())
            {
                var selector = Regex.Replace(match.Groups[1].Value, @"\s+", " ").Trim();
                if (!selector.Contains("data-theme")) continue;

                var targets = Regex.Matches(selector, @"(?:^|[\s>+~])([.#][A-Za-z][\w-]*)")
                    .Cast<Match>()
                ...
                bool stillUsed = targets.Any(target =>
                {
                    var name = target.Substring(1);
                    return handWritten.Any(page => Regex.IsMatch(page, $@"\b{Regex.Escape(name)}\b"));
                });

                if (!stillUsed) orphans.Add($"{selector} -> targets {string.Join(", ", targets)}");
            }

            Assert.That(orphans, Is.Empty,
                "theme.css light-mode rules no hand-written page can reach any more. Move each into the "
                + "React module (or shared ui/ component) that renders it, then delete it here:"
                + Environment.NewLine + "  " + string.Join(Environment.NewLine + "  ", orphans));
        }

        private static string StripCssComments(string css) => Regex.Replace(css, @"/\*.*?\*/", "", RegexOptions.Singleline);
