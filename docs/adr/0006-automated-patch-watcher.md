...
```
MainFormPresenter (owns watcher lifecycle)
...
- [ADR-0004: Presenter Decomposition for SRP](./0004-presenter-decomposition-srp.md) — its `PatchPresenter` was never wired live and was removed in build 2227; `MainFormPresenter` owns the watcher lifecycle
- `UI/Presenters/MainFormPresenter.cs` — presenter using the watcher service (`StartPatchWatcherAsync` / `OnPatchDetected`)
