...
Console.WriteLine($"Color: {status.StatusColor}");      // For UI display
```
...
`StatusService` is a stateless checker — there is no cache, events, or auto-refresh. Call `GetDetailedStatusAsync` whenever a fresh answer is needed. For live detection of Dota 2 updates, use `DotaPatchWatcherService` (`Core/Services/Update/`, ADR-0006), which watches `steam.inf`/`dota.signatures` and fires `OnPatchDetected`.
