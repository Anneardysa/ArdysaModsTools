...
| **Presenter** | Business logic, coordination | MainForm, ModOperations, Patch, Navigation, HeroGallery presenters |
...
| **MiscFormWebView**              | `misc_form.html`           | Miscellaneous mod selection UI               |
...
Remote feature gating notices (Skin Selector / Miscellaneous / Install ModsPack disabled by
`config/feature_access.json`) render as an in-shell confirm modal in `main_shell.html`
(`UIHelpers.ShowFeatureUnavailableAsync`) with a Join Discord action — there is no separate dialog form.
...
> **Removed 2026-07.** The runtime anti-tamper layer diagrammed below (`SecurityManager`,
> `AntiDebug`, `IntegrityCheck`; `SecureConfig`/`StringProtection` were never built) has been
> deleted — trivially bypassed and a source of antivirus hacktool false positives. See
> [ADR-0007](../adr/0007-security-anti-tamper-architecture.md). The startup sequence diagram earlier
> in this document showing a `SecurityManager.Initialize()` step is likewise historical. Remaining
> security code is asset-at-rest crypto only (`AssetCipher` / `EmbeddedAssetKey`).

_Historical (pre-2026-07):_
