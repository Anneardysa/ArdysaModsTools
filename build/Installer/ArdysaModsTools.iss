...
; ============================================================================
; UPDATE APPLIER (incremental updates)
; ============================================================================
; Standalone native exe. The app copies a verified copy of it into its staging folder and runs it
; from there to swap the changed files in — see Core\Services\Update\DeltaUpdateService.cs. Must be
; installed here (and hashed in files.json) or the very first delta has nothing to launch.
;
; skipifsourcedoesntexist: only release.yml publishes the Native-AOT applier into the publish tree, so
; a developer compiling this script after a plain `dotnet publish` would otherwise hit a hard error on
; this one line. Releases always have it; an install without it still self-heals, because every delta
; re-downloads the applier into staging and runs it from there, never from {app}.
Source: "..\..\publish\installer\tools\updater\AMT.Updater.exe"; DestDir: "{app}\tools\updater"; Flags: ignoreversion skipifsourcedoesntexist
...
Source: "..\..\tools\hllib\msvcr100.dll"; DestDir: "{app}"; Flags: ignoreversion
Source: "..\..\tools\hllib\msvcp100.dll"; DestDir: "{app}"; Flags: ignoreversion
...
// Reads the WebView2 Runtime EdgeUpdate 'pv' value from one hive/path.
// A 'pv' of '' or '0.0.0.0' is written for an uninstalled/broken runtime, not a
// usable one, so both are treated as absent.
function HasValidWebView2Pv(RootKey: Integer; SubKey: String): Boolean;
var
  Version: String;
begin
  Result := False;
...
begin
  // Each hive is checked independently — a present-but-empty key must not stop the
  // next location from being tried. EdgeUpdate is a 32-bit app, so a per-machine
  // runtime registers under WOW6432Node; HKCU covers a per-user install.
  Result :=
    HasValidWebView2Pv(HKLM, 'SOFTWARE\WOW6432Node\Microsoft\EdgeUpdate\Clients\{F3017226-FE2A-4295-8BDF-00C3A9A7E4C5}') or
    HasValidWebView2Pv(HKLM, 'SOFTWARE\Microsoft\EdgeUpdate\Clients\{F3017226-FE2A-4295-8BDF-00C3A9A7E4C5}') or
    HasValidWebView2Pv(HKCU, 'SOFTWARE\Microsoft\EdgeUpdate\Clients\{F3017226-FE2A-4295-8BDF-00C3A9A7E4C5}');
end;
