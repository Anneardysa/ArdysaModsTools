...
## [2.4.1-beta] (Builds 2364–2368)

### Added

- **Added remote generation cooldown toggle via feature access configuration** (2368): Integrated remote on/off switch and custom duration support for Skin Selector generation cooldown via Cloudflare R2 feature access configuration.
...
- **Added multi-tier item dataset extraction for package sync** (2361): Introduced `ExtractModItemsGameAsync` and dynamic VPK stamp resolution in `ItemsGameSyncService` to reliably detect and compare `items_game.txt` across live sessions, staging packages, and standard installations.
...
- **Optimized Cloudflare R2 cache headers across releases, deltas, and manifests** (2356): Added long-lived immutable caching to asset packages and delta files to prevent redundant CDN re-fetching, while setting responsive cache headers on manifest feeds.
- **Streamlined dataset localization tooling with automatic game path discovery** (2356): Added CLI options (--vpk, --root, --output), automatic Steam library detection for Dota VPK files, and integrated root path forwarding in the toolbox launcher.
...
- **A single failed incremental-update download no longer disables auto-update for every future release** (2351): A staging failure (network drop, tampered CDN bytes) marked the version failed and persisted it, and `CanAutoUpdate` treated ANY recorded failure — for any version, with no expiry — as a reason to stop offering the incremental path at all; `ClearFailedVersion` had no caller, so the only fix was deleting `update_state.json` by hand. Staging never touches the install directory, so there was nothing to loop-guard against. `CanAutoUpdate` now only checks the target version's own failure/loop-guard state, and a staging or applier-launch failure no longer calls `MarkVersionFailed` at all.
- **A failed relaunch after a successful update is no longer silent, and no longer runs the app elevated** (2351): `AMT.Updater.exe` wrote `OK: Updated to v…` before attempting to restart the app, so a relaunch failure (missing exe, `Process.Start` throwing) left no trace anywhere and the user found no app running with nothing to diagnose. It also relaunched via `Process.Start` with no `Verb`, inheriting the applier's elevated token whenever the install path required `runas` — silently promoting the restarted app to administrator. Relaunch now retries once, reports failure explicitly (keeping the staging log instead of deleting it), and launches through `explorer.exe` when elevated so the new process gets the user's normal token back.
- **The app can no longer stay open past the applier's 60-second wait and get left with neither the update nor a running app** (2351): `Application.Exit()` honors `FormClosing`'s `e.Cancel`, and the main window cancels its close while any operation is running, awaiting an unbounded shutdown. If that outlasted the applier's wait for the parent PID, the applier aborted the update *and* refused to relaunch. A 20-second backstop now force-exits if the normal close hasn't completed by then.
- **A stale `.staged-ok` marker can no longer survive into a restaged update** (2351): Clearing the staging folder before a restage swallows per-folder delete failures (a locked file, an AV holding a handle), so a marker from an incomplete previous attempt could survive — the applier only checks that the marker exists, not what it means. `StageAsync` now deletes it explicitly before the first byte downloads.
- **Finished updates no longer leave their downloaded files sitting in `%LocalAppData%`** (2351): The staging folder was only ever cleared at the *start* of the next update attempt. `DeltaUpdateService` gained `ClearStagingRoot()`, called once a clean apply's outcome has been reported.
- **An injected pick's "sound" asset_modifier no longer redirects to nothing** (2348): A `sound` type redirects one soundevent NAME to another (`Hero_Sven.GreatCleave` → `Hero_Sven.GreatCleave.ti7`), but both names are sibling entries inside one compiled `.vsndevts_c`, not two file paths — there is no resource-loader path-swap trick for it the way there is for particle/model, and `kv3.py` cannot rebuild the block (every shipped `.vsndevts_c` is KV3 v5, whose values sit in a buffer `kv3.Cursor` never marks writable). `sound_forge.py` now does what a modder does by hand — decompile the hero's compiled soundevents, splice the modifier event's body onto the base event verbatim, recompile — using the same Source2Viewer-CLI/resourcecompiler toolchain `vmdl_forge.py` already runs. Whatever it can't resolve (no Workshop Tools DLC, or the event isn't in either candidate file) still falls back to the hand-curated `soundfix/<hero>/` bundle.
- **An injected pick's animations no longer re-pick and stutter on a body-redirecting hero** (2348): Whatever lands on the hero's own body path — a persona/hero_base pick's model, or an `entity_model` modifier's target — ships that body's own activity table verbatim, and that table is built for a dedicated loadout slot: most of it gated behind modifiers the base body's own sequences never ask for. Harmless on one-shot activities, but on a looping one like `ACT_DOTA_IDLE` two candidates left tied makes the client re-roll every loop restart — a visible pop while standing still. `vmdl_forge.conform_body` now conforms the redirected body's activity table to the base body's before it ships, the fix a real modder makes by hand in ModelDoc.
- **The Hero Assets injector no longer decides a pick's scalars by where the pick happened to write them** (2346): `merge()` wrote a fixed header whitelist and copied the pick's raw lines from its first sub-block down, so the same key survived when a pick wrote it below that sub-block and was dropped when it wrote it above — `match_cycle_to_parent`, which says whether a wearable's animation cycle follows the hero's, lived on 2 picks and died on 409. Every scalar is now decided by meaning: the pick's own content comes along (animation, attack attachments, particle folder and snapshot), while store metadata is dropped by name wherever it sits, including `expiration_date` on 130 picks — a rental that names the day the injected item stops rendering.
- **An injected cosmetic no longer reverts to vanilla when the hero changes body** (2346): Night Stalker reads his night form off `model_player1` and Tiny his three growth stages off `model_player1..3`, and those numbered keys were dropped from the merged def entirely, so 74 picks shipped one body out of two (or four). `merge()` and `file_plan()` now apply `model_player`'s own rename to each numbered slot, and `skin_plan()` resolves a style's material group against every body — the night `.vmdl` carries its own parallel groups, so Frightful Revelries at style 1 was styled by day and base by night. Verified by re-running all 11275 injectable (pick, style) pairs: 75 model files and 6 materials gained, none lost, no new warnings.
- **A styled inject that correctly writes no material now says why instead of looking like a failure** (2346): Three of `skin_plan()`'s outcomes are right and empty — the model declares no groups, the skin is past the last group, or group N repeats group 0 — and a mod tree with one model file and no `.vmat_c` is indistinguishable from a material swap that went missing. Those cases now report their reason as information rather than a warning (a TI10 Cavern Crawl set's style 2 is the base material plus its own particles, which is why it needs no material), and the page's inject result card prints the style it built, which it had never shown at all.
- **The Hero Assets injector now honours a style's `skin`, not just its `model_player`** (2345): A style in `items_game.txt` flips two switches — a model path and a `skin` index into that model's material groups — but an injected `default_item` def can select neither, so both have to be baked in by file placement. `inject.py` only ever handled the model, which shipped the right mesh wearing the wrong material (Invoker's Apex hair in Dark Artistry's colours). `skin_plan()` now resolves the group from the compiled `.vmdl_c` and writes group N's material bytes at group 0's paths, the same rename trick the model already used. The Hero Assets page gained a Model references panel listing a pick's material groups and its RERL references, flagging any the VPK does not contain.
...
- **The `/commit` skill now states the `scripts/dataset/hero_assets/` exclusion explicitly instead of relying on the agent to recall it** (2352): The rule already existed as a memory note, but nothing in the skill's own read-state, changelog, or staging steps said to exclude that local-only dev tool from a commit, leaving it to be caught by chance. `SKILL.md` now names the exclusion up front in step 1 and repeats it at the two steps it actually affects (changelog, staging).
...
- **Added multi-layered auto-update loop protection and persistent update state** (2326): `DeltaUpdateService` now persists failed versions, attempt counts, and failure reasons in `%LocalAppData%\ArdysaModsTools\update\update_state.json` across app restarts and staging cleanups. Added normalized version matching, post-apply version advancement verification in `ReportLastApplyOutcome` to prevent rapid restart loops when binary versions stagnate, and attempt throttling in `IsAutoUpdateLoopGuarded`.
- **Added descriptive error notifications for update failures across dialog and shell** (2326): `UpdateAvailableDialogWebView` and `MainFormPresenter` now communicate explicit, actionable failure reasons (e.g. hash verification errors, manifest timeouts, UAC declination, or version stagnations) via in-shell toast notifications and styled high-contrast error banners in `update_available.html` rather than generic failure placeholders.
...
- **Updated auto-update troubleshooting guide with comprehensive fix steps** (2326): Expanded `docs/TROUBLESHOOTING.md` with step-by-step resolution guides including manual direct downloads, staging cache clearing, background process termination, UAC/Antivirus exclusions, and diagnostic log inspection.

## [2.2.23-beta] (Builds 2318–2323)

### Reverted

- **Reverted hero skin corruption guard and baseline item index changes** (2322): Reverted commits 706db8c..56615a8 due to application malfunction.

### Changed

- **Temporarily disabled Play button and Package Sync status in main_shell** (2321): Removed Play button element and Package Sync status check item from `main_shell` UI layout, updated `SetupVerificationService` and `MainFormPresenter` logic to disable Play launch/repair flow and sync warnings, and updated localized reason strings across all locale catalogs.
...
- **Added Download Server preference in Settings** (2316): users can now choose between Auto (Smart Selection), Asia (Cloudflare R2), and EU / US (Backblaze B2). SmartCdnSelector pins the preferred CDN at position 0 while preserving session circuit-breaker protection.
...
- **Protected asset split fails closed again** (2264): builds 2261–2263 dropped the
  `ProtectedVpkStore.IsMounted(targetPath)` condition in `HeroGenerationService`, so a generation
  always moved protected-set assets into `game/mod`. That folder only loads when the *installed*
  gameinfo mounts it, and `Ensure()` merely creates and superhides the directory — it cannot make
  Dota mount it. A user who had updated AMT but not yet run Patch Update got their cosmetics moved
  into an unmounted package: silently missing in game, with no error anywhere. The guard is
  restored; when the mount is absent every asset stays in the single main package, exactly the
  pre-split behavior.
- **Diagnostics land in a writable file** (2264): build 2255 moved ~60 pipeline diagnostics onto
  `FallbackLogger.LogFileOnly`, which writes next to the exe. The installer targets `{autopf}`
  (Program Files) and `app.manifest` is `asInvoker` — a manifested process gets no UAC file
  virtualization, so every write threw into the swallow-all catch and the diagnostics were
  discarded rather than relocated. `Program.Main` now points the log at
  `%LocalAppData%\ArdysaModsTools\ardysa_fallback.log` before anything logs.

### 🛠 Refactor (build 2261-2263)

- Internal update to package asset separation and store handling. Details intentionally omitted.
  `MoveProtected` keeps its required path list — the whole-tree enumeration branch added alongside
  it had no caller.
...
- **Incremental ("delta") app updates — the ~200 MB installer re-download is gone** (2234): Every update forced a full installer download regardless of how little changed. The app ships as a self-contained .NET 8 publish tree (**174 MB, 528 files**) of which **~167 MB is the .NET runtime, byte-identical across releases** — only `ArdysaModsTools.dll`, its apphost and `Assets/` actually change: **3.9 MB across 47 files** in the worst case where all of them change at once. Each release now publishes a per-file SHA-256 manifest (`releases/<version>/files.json` on R2) alongside the file tree it describes; the client hashes its own install folder against it and downloads **only the entries that differ**, then applies them out-of-process. See [ADR-0012](docs/adr/0012-incremental-delta-updates.md).
   - **Manifest contract**: deliberately the _same schema_ as `Assets/asset_hashes.json` (ADR-0010), so `AssetHashManifestService.ParseManifest` and `AssetHashVerifier` are reused unchanged. `releases.json` gains one **additive** field per release — `"files": "<manifest URL>"` — which older clients ignore; its absence simply means "full download only". The manifest URL is the **single source of truth** for where a release lives: the file base (`…/files/`) and the previous version's manifest are _derived_ from it, so the manifest and the files it describes can never point at different places.
   - **Client** (`Core/Services/Update/DeltaUpdateService.cs`): `PrepareAsync` fetches the manifest, hashes the install tree, and diffs (deletions come from the _previous_ version's manifest, `old keys − new keys`; when that manifest is purged the answer is "delete nothing", never a guess). `StageAsync` downloads each file into `%LocalAppData%\ArdysaModsTools\update\<version>\`, **every one verified against the manifest hash** by `ResumableDownloadService` (ADR-0009/0010); only after _all_ files verify are `apply.json` and then the `.staged-ok` marker written, so a half-finished staging folder can never be applied.
...
- **The install rollback and the update check had zero coverage** (2231): `InstallSnapshot` — the thing standing between a failed ModsPack install and a user left with _no_ mods — was untested. Three new NUnit cases pin it: dispose-without-commit restores the previous `pak01_dir.vpk` + `ModsPack.hash` byte-for-byte and leaves no `.bak`; `Commit` keeps the new pair and drops the backups; and on a **fresh** install (no prior files) a rollback clears the slot rather than leaving a half-written VPK with a hash marker vouching for it. Four more pin `ModsPackUpdateService.CheckForUpdateAsync`: the null-installer guard, empty/whitespace paths, the **no-local-install → false** semantics established above (which returns before any network call), and that a cancelled token surfaces as `OperationCanceledException` rather than a quiet "no update". 715 tests green.

### 🐛 Fixed (build 2230)

- **Disable Mods wrote to the game folder before the download that could fail** (2230): `DisableModsAsync` trimmed the pinned `ModPatchLine` out of `dota.signatures` **first**, and only then downloaded the clean `gameinfo_branchspecific.gi` it needs to restore. When every CDN was unreachable — the _common_ failure — the user was left with a stripped signature file next to a still-modded game config: the mismatched pair `StatusService.CheckSignaturesPatched` flags as an invalid (VAC-risk) patch. The signatures read and the download now both happen **before the first write**, the same ordering rule build 2228 established on the patch path, which is also why the two writes below it don't need a `FileTransaction` to be safe.
- **A failed restore reported "Mods disabled successfully"** (2230): if writing the downloaded game config threw (file locked by Dota 2 or antivirus, disk full), the `catch` only logged, execution fell through to the success log and the method returned `true` — with the **modded config still installed and mods still active**. Worse, on the _Delete Permanently_ route `MainFormPresenter` took that `true` at face value and deleted `game/_ArdysaMods`, leaving a game config mounting a VPK that no longer existed. A failed restore now falls through to the delete-the-config fallback (Steam then restores a clean one, mods are off either way), and if _that_ delete also fails the method returns `false` — the one state where mods genuinely are not disabled.
...
- **Main shell — a stale tooltip lingered over "Checking…"** (2227): `showChecking()` updated the status dot and text but left the previous status' `title` on both, so hovering the checking state showed the _old_ status' description. Both titles are now cleared.
...
- **Language selector in Settings**: A new **Language** control under _Appearance_ lets you switch language live — the shell, Settings, and the localized dialogs re-translate in place via the `CultureChanged` event, with no restart.
...
- **In-shell "Update Required" prompt**: The native Yes/No _Update Available_ `MessageBox` shown by the Patcher when Dota 2 has been updated is replaced with the in-shell monochrome confirmation modal (`ShowShellConfirmAsync`), matching the rest of the shell. The classic form still falls back to a native box. New `patch.statusPrompt.*` locale keys (eyebrow / heading / body / confirm) were added across all 8 languages.

### 🐛 Fixed

- **Skin Selector — stale / ghost assets between runs**: Generation previously merged each run's custom set assets directly into the cached _pristine_ base extraction, leaving those files behind so they bled into ("ghosted" onto) later, unrelated generations. Each run now copies the cached extraction into its own temp folder and patches that disposable copy, keeping the cache clean. The base-ready cache marker was bumped (`.ready` → `.ready2`) to force a one-time clean re-extract on existing installs.
